...
class Sciene {
    ...
    static coordinate = {left:[100,400,100,400,100,400,100,400],top:[400,400,550,550,700,700,850,850]}
    static unlock_sciene = []

    static clear(){

    }
    //更新建筑数量
    static updateSciene(id,type){
        if(type ==0){
            if(DB.data.sciene[id][0] && Global.mainFace.id ==0){
                Sciene.unlock_sciene.push(id+101);
    ...
    static updateScieneButton(){
        for(let i=0;i<Sciene.unlock_sciene.length;i++){
            Scene.open("app-ui-scieneButton", Global.mainFace.node,null, {id:Sciene.unlock_sciene[i]});
        }
 ...
 */
const open = () => {
    Global.mainFace.node = Scene.open("app-ui-sciene", Scene.root);
    Global.mainFace.id = 0;
    //显示解锁的科技按钮
    for(let i=0; i<DB.data.sciene.length;i++ ){
        if(DB.data.sciene[i][0] && !DB.data.sciene[i][1]){
            Sciene.unlock_sciene.push(i+101);
...
const initSciene = () => {
    ...
    DB.init("sciene", tempDB);
};
//注册页面打开事件
AppEmitter.add("intoSciene",(node)=>{
    open();
});
...
for(let i = DB.data.sciene.length - 1; i >= 0; i--){
    for(let j =0;j<2;j++){
        DB.emitter.add(`build.${i}.${j}`, ((x,y) => {
            return ()=>{
                Sciene.updateSciene(x,y)
            }
        })(i,j));
    }
}   