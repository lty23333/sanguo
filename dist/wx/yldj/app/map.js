    ...
    if (Global.mainFace.id == 4) {
      for (let i = 0; i < DB.data.map.guard.length; i++) {
        if (DB.data.map.guard[i][0]) {
          Map.city_sprite[i] = Scene.open("app-ui-city", Global.mainFace.node, null, {
            id: DB.data.map.guard[i][0],
            index: i
          });
        }
      }
...
class WCity extends Widget {
  ...
  choose(id) {
    ...
    Scene.open("app-ui-fightWindow", this.backNode, null, {
      id: this.props.id,
...
class WfightWindow extends Widget {
  ...
  fight() {
    ...
    AppEmitter.emit("intoFight", {
      fighter: [own, enemy],
      city: this.props.id,
      enemyType: this.props.index
    });
  }
